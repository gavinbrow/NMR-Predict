...
/**
 * The chromatographic peak table. Columns: RT (3 dp), Height, Area, Area %,
 * Width (rtEnd − rtStart, 3 dp), Base m/z, Name. Every header is sortable
 * (click to toggle asc/desc, chevron on the active column). Clicking a row
 * calls `onRowClick(peak)` so the page can move the RT cursor; the selected
 * row is highlighted with `bg-muted`. The `Name` cell is an inline-editable
 * `Input` calling `onRename(id, name)`.
 *
...
type SortKey = "rtApex" | "height" | "area" | "areaPct" | "width" | "basePeakMz" | "name";

const ROW_H = 32;
const OVERSCAN = 10;

export function ChromPeakTable({ peaks, selectedPeakId, onRowClick, onRename, onDelete }: ChromPeakTableProps) {
