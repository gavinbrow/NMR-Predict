// The DSC workspace's export menu: CSVs of the processed curves / transitions
// / summary, a full Excel workbook with native editable charts, a PDF report,
// and the `.dscproj` project file (which can be opened again from here).
// Mirrors `components/tga/ExportMenu.tsx` — see that file for the pattern
// this was cloned from.
//
// The Excel and PDF exports embed the publication figure exactly as the user
// styled it — the page passes the same `FigureData`/`FigureOptions` the
// Figure tab renders, and it's rasterized off-screen at full resolution, so
// the report matches the preview regardless of which tab is on screen.
...
import { buildSummaryRows, dscMetrics } from "@/lib/dsc/compare";
import {
...
import { useDscStore } from "@/lib/dsc/store";
...
export function ExportMenu({
  ...
  /** Called with the figure options a `.dscproj` carried, so the host can
   *  restore the exact styling the project was saved with. */
  onFigureOptionsRestored: (options: FigureOptions | null) => void;
}) {
  const { runs, materials, params, rawState, loadProject } = useDscStore();
  ...
  const baseName = exportRuns[0]?.fileName.replace(/\.[^.]+$/, "") || "dsc";
   ...
   * and a background tab delivers none: without the race, switching tabs
   * while a workbook is building leaves the export hanging on a spinner
   * until the page is looked at again. Ten seconds is far longer than a real
   * raster takes.
   */
  ...
  const openProject = (file: File) => {
    void run("Opening the project", async () => {
      ...
      const { state, figureOptions: saved } = deserializeDscProject(text);
      loadProject(state);
    ...
    <>
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="w-64">
          ...
          <DropdownMenuItem
            className="text-xs"
            onClick={() => run("Curves CSV", () => downloadCurvesCsv(exportRuns, params, baseName))}
          >
          ...
          <DropdownMenuItem
            className="text-xs"
            onClick={() =>
              run("Transitions CSV", () => downloadTransitionsCsv(exportRuns, params, baseName))
            }
          >
            <Table2 className="mr-2 h-3.5 w-3.5" />
            Transitions
          ...
          <DropdownMenuItem
            className="text-xs"
            onClick={() =>
              run("Project save", () => downloadDscProject(rawState, figureOptions, baseName))
            }
          >
      ...
      <input
        ref={fileInputRef}
        type="file"
        accept=".dscproj,application/json"
        className="hidden"
        onChange={(e) => {
