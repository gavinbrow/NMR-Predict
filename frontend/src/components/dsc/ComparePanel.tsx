...
//
// A presentational recharts view over `lib/dsc/compare.ts`'s pure builders —
// colours and strokes are explicit props rather than Tailwind classes so the
// SVG serializes faithfully if it is ever captured for an export. Clones
// `components/tga/ComparePanel.tsx` verbatim, DSC types swapped in.
...
import type { DscBarDatum, DscMetric, DscMetricKey } from "@/lib/dsc/compare";
...
export function ComparePanel({
  ...
  bars: DscBarDatum[];
  metrics: DscMetric[];
  metricKey: DscMetricKey;
  onMetricChange: (k: DscMetricKey) => void;
            ...
            <BarChart data={bars} margin={{ top: 12, right: 20, bottom: 28, left: 8 }}>
              ...
              <Tooltip
                formatter={(value: number, _name, item) => {
                  const d = item?.payload as DscBarDatum | undefined;
