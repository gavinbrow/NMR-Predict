...
//
// One material per file by default (`DAC1`/`DAC2`/`DAC3` land in three
// materials from three files); from here the user can rename it, move a run
// into another material, merge two, or split a selection into a new one. The
// mean ± SD shown on each card is the same number the compare chart bars,
// computed by `lib/dsc/compare.ts`. Clones `components/tga/MaterialsPanel.tsx`
// — same structure, DSC store/types swapped in.
...
import { metricValue, summarize, type DscMetric } from "@/lib/dsc/compare";
import { useDscStore } from "@/lib/dsc/store";
import type { DscRunAnalyzed } from "@/lib/dsc/store";
import type { DscMaterial } from "@/lib/dsc/types";
...
export function MaterialsPanel({
  ...
  metric: DscMetric | undefined;
  /** Run ids currently selected, for "new material from selection". */
  selectedRunIds: string[];
  onSelectRun: (runId: string) => void;
}) {
  const { materials, runById, renameMaterial, createMaterialFrom, deleteMaterial } = useDscStore();
    ...
    <div className="flex flex-col gap-3">
      ...
      {materials.map((m) => (
        <MaterialCard
          ...
          runs={m.runIds.map((id) => runById.get(id)).filter((r): r is DscRunAnalyzed => !!r)}
          metric={metric}
...
function MaterialCard({
  ...
  material: DscMaterial;
  allMaterials: DscMaterial[];
  runs: DscRunAnalyzed[];
  metric: DscMetric | undefined;
  ...
  const { moveRun, mergeMaterials } = useDscStore();
